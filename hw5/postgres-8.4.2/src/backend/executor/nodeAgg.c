...
//
//  * Returns YES(1) if sort was successful, or NO(0) if the nested
...
approx_agg_advance_iter(AggState *aggstate, Agg* agg) {
    ...
    if (pos >= agg->approx_nkeep || aggstate->topK[pos].tuple == NULL)// || aggstate->topK[pos].count <= 0)
        return NULL;
    elog(LOG,"count %u: ", aggstate->topK[pos].count);
    aggstate->iterPos++;
